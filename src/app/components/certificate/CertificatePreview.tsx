...
export function CertificatePreview({
  ...
  return (
    <div
      className={`certificate-preview relative overflow-hidden rounded-2xl border border-[#81d8d0]/35 bg-white text-[#1e1e1e] shadow-2xl shadow-black/25 ${
        compact ? "p-4" : "p-5 sm:p-6"
      }`}
    >
      <div className="pointer-events-none absolute inset-0 certificate-preview-grid opacity-[0.06]" />
      <div className="pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full bg-[#0abab5]/10 blur-2xl" />
      <div className="pointer-events-none absolute -bottom-10 -left-10 h-28 w-28 rounded-full bg-[#81d8d0]/10 blur-2xl" />
      <div className="absolute left-0 top-4 w-full -rotate-12 bg-gradient-to-r from-[#0abab5] via-[#81d8d0] to-[#3d3d3d] py-1 text-center text-[8px] font-black uppercase tracking-[0.28em] text-white opacity-90">
      ...
      <div className={`relative text-center ${compact ? "py-5" : "py-7"}`}>
        ...
        <div className="mx-auto my-4 h-1 w-24 rounded-full bg-gradient-to-r from-[#0abab5] via-[#81d8d0] to-[#3d3d3d]" />
        <p className="text-sm text-slate-500">This is to certify that</p>
        <p
          className={`mx-auto mt-2 max-w-lg border-b border-[#d4af37] pb-2 font-black text-[#1e1e1e] ${
      ...
      <div className="relative grid grid-cols-3 items-end gap-3 text-center text-[9px] font-black uppercase tracking-[0.12em] text-slate-500">
        <div className="border-t border-slate-300 pt-2 text-left">
          Completion
          <span className="mt-1 block text-[11px] normal-case tracking-normal text-[#1e1e1e]">
            {dateLabel}
          </span>
        </div>
        <div className="mx-auto grid h-14 w-14 place-items-center rounded-full border-2 border-[#81d8d0] bg-[#fff7df] text-[8px] leading-tight text-[#1e1e1e] shadow-[0_0_0_4px_rgba(242,184,75,0.12)]">
          SkillVane
          <br />
          Verified
        </div>
        <div className="border-t border-slate-300 pt-2 text-right">
          <span className="block font-serif text-base normal-case tracking-normal text-[#1e1e1e]">
            Shaik Saidhul
          </span>
          Lead Instructor
        </div>
