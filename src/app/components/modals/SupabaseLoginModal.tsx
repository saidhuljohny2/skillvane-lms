      ...
      <div className="relative w-full overflow-hidden rounded-t-2xl border border-white/[0.09] bg-[#0a1522] shadow-[0_30px_90px_rgba(0,0,0,0.5)] sm:max-w-md sm:rounded-[1.5rem]">
        ...
        <div className="relative flex items-center justify-between border-b border-white/10 px-5 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#2dd4a6]"><Lock className="h-5 w-5 text-[#04110d]" /></div>
            <div>
        ...
        <form onSubmit={submit} className="relative space-y-4 px-5 py-5">
          ...
          {message && (
            <div className={`rounded-lg border p-3 text-sm ${isError ? "border-red-500/30 bg-red-500/10 text-red-300" : "border-[#18c29c]/30 bg-[#18c29c]/10 text-[#9cf8dd]"}`}>{message}</div>
          )}
          {mode === "signup" && (
            <label className="block text-sm font-semibold text-slate-200">Full Name
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Enter your full name" className="mt-1.5 w-full rounded-xl border border-white/[0.09] bg-white/[0.04] px-4 py-3 text-sm text-white placeholder-slate-500 focus:border-[#2dd4a6]/40 focus:outline-none" />
            </label>
          )}
          <label className="block text-sm font-semibold text-slate-200">Email
            <input type="email" autoComplete="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="your.email@example.com" className="mt-1.5 w-full rounded-xl border border-white/[0.09] bg-white/[0.04] px-4 py-3 text-sm text-white placeholder-slate-500 focus:border-[#2dd4a6]/40 focus:outline-none" />
          </label>
          {mode === "signup" && (
            <label className="block text-sm font-semibold text-slate-200">Phone
              <input type="tel" autoComplete="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value.replace(/\D/g, "").slice(0, 10) })} placeholder="10-digit mobile number" className="mt-1.5 w-full rounded-xl border border-white/[0.09] bg-white/[0.04] px-4 py-3 text-sm text-white placeholder-slate-500 focus:border-[#2dd4a6]/40 focus:outline-none" />
            </label>
          )}
          {mode !== "reset" && (
            <label className="block text-sm font-semibold text-slate-200">Password
              <input type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="Minimum 6 characters" className="mt-1.5 w-full rounded-xl border border-white/[0.09] bg-white/[0.04] px-4 py-3 text-sm text-white placeholder-slate-500 focus:border-[#2dd4a6]/40 focus:outline-none" />
            </label>
          )}
          <button type="submit" disabled={loading || !isSupabaseConfigured} className="w-full rounded-xl bg-gradient-to-r from-[#18c29c] to-[#2f80ed] py-3 text-sm font-black text-white transition-all hover:shadow-xl hover:shadow-[#18c29c]/20 disabled:cursor-not-allowed disabled:opacity-50">
            {loading ? "Please wait..." : mode === "login" ? "Sign In" : mode === "signup" ? "Create Account" : "Send Reset Link"}
          </button>
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-center text-sm">
            {mode !== "login" && <button type="button" onClick={() => changeMode("login")} className="text-slate-400 hover:text-[#8df5d7]">Back to login</button>}
            {mode === "login" && <button type="button" onClick={() => changeMode("signup")} className="text-slate-400 hover:text-[#8df5d7]">Create an account</button>}
            {mode === "login" && <button type="button" onClick={() => changeMode("reset")} className="text-slate-400 hover:text-[#8df5d7]">Forgot password?</button>}
          </div>
